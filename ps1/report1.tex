...
For each of the four datasets (I, II, III, IV), the following sample
statistics were computed: the mean of \texttt{x}, the mean of \texttt{y},
the variance of \texttt{x}, the variance of \texttt{y}, the correlation
between \texttt{x} and \texttt{y}, and the $R^2$ implied by these
statistics alone (using the identity $R^2 = r_{xy}^2$ for simple linear
regression).
...
For each dataset, OLS estimates $\hat{\beta}_0$ and $\hat{\beta}_1$
were obtained from a regression of \texttt{y} on \texttt{x} using
\texttt{lm()}.
...
The following code loads the Connecticut property data, constructs the
true assessed value, expresses both assessed value and sale amount in
\$1,000s, and filters the sample to single-family homes in New Haven
